            ...
            <table class="min-w-full border-collapse border border-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700 border-b">Tanggal Pengajuan
                        </th>
                ...
                <tbody>
                    @forelse($pengajuan as $index => $izin)
                        <tr class="{{ $loop->odd ? 'bg-gray-50' : 'bg-white' }}">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 border-b">
                                {{ $izin->created_at->format('d-m-Y') }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 border-b">
                                <form action="{{ route('admin.persetujuan.izin', $izin->id) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="status" value="Disetujui">
                                    <button type="submit"
                                        class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">Setujui</button>
                                </form>
                                <form action="{{ route('admin.persetujuan.izin', $izin->id) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    ...
                                    <button type="submit"
                                        class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400">Tolak</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">
                                Tidak ada pengajuan izin atau cuti yang masuk.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
